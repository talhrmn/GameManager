...
class StatisticsRepository(BaseRepository[StatisticsBase, StatisticsDBOutput]):
    ...
    async def get_all_user_stats(self, user_id: str) -> Optional[StatisticsDBOutput]:
        ...
        Returns:
            Optional[StatisticsDBOutput]: The user's monthly stats if found, None otherwise
    ...
    async def increment_user_stats(self, user_id: str, user_inc: Stats) -> Optional[StatisticsDBOutput]:
        ...
        Returns:
            Optional[StatisticsDBOutput]: The updated user if successful, None otherwise
        ...
        try:
            ...
            return await self.get_one_by_query({"user_id": user_id})
        except Exception as e:
    ...
    async def upsert_monthly_stats(self, user_id: str, month: str, user_inc: MonthlyStats) -> Optional[
        ...
        try:
            if not ObjectId.is_valid(user_id):
                return None
            user_query = {"user_id": user_id, "monthly_stats.month": month}
            projection = {"monthly_stats.$": 1}
            found = await self.get_one_by_query(user_query, projection, dump_model=False)
            if found and found.get("monthly_stats"):
                update_fields = ["profit", "tables_played", "hours_played", "games_won", "games_lost"]
                inc_dict = {
    ...
    async def update_monthly_stats(self, user_id: str, month_str: str, month_win_rate: float) -> None:
        ...
        try:
            ...
            result = await self.collection.update_one(
                {"user_id": user_id, "monthly_stats.month": month_str},
