...
def main():
    ...
    for i in range(1, NUM_TABLES + 1):
        ...
        # Create players - FIXED: Ensure creator is always included
        creator_idx = random.randint(0, NUM_USERS - 1)
        creator_id = str(users[creator_idx]["_id"])

        # Start with creator in the players list
        num_additional_invited = random.randint(max_players - 3,
                                                max_players + 1)  # -3 to account for creator + at least 1 more

        # Get other players (excluding creator)
        other_player_indices = [idx for idx in range(NUM_USERS) if idx != creator_idx]
        additional_player_indices = random.sample(
            other_player_indices,
            min(num_additional_invited, len(other_player_indices))
        )

        # Combine creator with additional players
        all_player_indices = [creator_idx] + additional_player_indices

        players = []
        for player_idx in all_player_indices:
        ...
        if table_status != "cancelled":
            ...
            if confirmed_count < 2:
                # Convert some declined/invited to confirmed (but don't touch creator who's already confirmed)
        ...
        if game_date is not None:
            ...
            # But ensure creator always shows up if they're confirmed
            attending_players = []
            for player in confirmed_players:
                if player["user_id"] == creator_id:
                    # Creator always shows up if confirmed
                    attending_players.append(player)
                elif random.random() < 0.9:  # 90% show rate for others
                    attending_players.append(player)

            # Ensure at least 2 players if we're creating a game
            if len(attending_players) < 2 and len(confirmed_players) >= 2:
                # Add more players, prioritizing those who haven't been added yet
                missing_players = [p for p in confirmed_players if p not in attending_players]
                needed = min(2 - len(attending_players), len(missing_players))
                attending_players.extend(missing_players[:needed])
            ...
            if game_status == "completed" and game_players:
                # Create a more realistic distribution with clear winners and losers
                num_players = len(game_players)

                # Determine winner/loser distribution (roughly 30% winners, 70% losers)
                num_winners = max(1, int(num_players * 0.3))
                num_losers = num_players - num_winners

                # Shuffle players to randomly assign winners/losers
                shuffled_players = game_players.copy()
                random.shuffle(shuffled_players)

                winners = shuffled_players[:num_winners]
                losers = shuffled_players[num_winners:]

                # Calculate buy-ins for each group
                total_winner_buyins = sum(sum(b["amount"] for b in p["buy_ins"]) for p in winners)
                total_loser_buyins = sum(sum(b["amount"] for b in p["buy_ins"]) for p in losers)

                # Distribute losses first (losers get 10-80% of their buy-in back)
                total_losses = 0
                for player in losers:
                    player_buy_in = sum(b["amount"] for b in player["buy_ins"])
                    # Losers cash out between 10-80% of their buy-in
                    cash_out_percentage = random.uniform(0.1, 0.8)
                    cash_out = int(player_buy_in * cash_out_percentage)
                    player["cash_out"] = cash_out
                    player["net_profit"] = cash_out - player_buy_in
                    total_losses += (player_buy_in - cash_out)

                # Distribute the losses among winners
                remaining_winnings = total_losses
                for i, player in enumerate(winners):
                    player_buy_in = sum(b["amount"] for b in player["buy_ins"])

                    if i == len(winners) - 1:
                        # Last winner gets whatever is left
                        winnings = remaining_winnings
                    else:
                        # Distribute winnings somewhat randomly but ensure everyone wins something
                        max_possible_win = min(remaining_winnings - (len(winners) - i - 1) * 10,
                                               int(total_losses * 0.6))  # No single player wins more than 60% of total losses
                        min_win = 10  # Minimum win amount
                        winnings = random.randint(min_win, max(min_win, max_possible_win))
                        remaining_winnings -= winnings

                    cash_out = player_buy_in + winnings
                    player["cash_out"] = cash_out
                    player["net_profit"] = winnings

                # Verify total is correct (should equal total_buy_ins)
                total_cash_outs = sum(p["cash_out"] for p in game_players)
                if total_cash_outs != total_buy_ins:
                    # Adjust the biggest winner to balance
                    biggest_winner = max(winners, key=lambda p: p["net_profit"])
                    adjustment = total_buy_ins - total_cash_outs
                    biggest_winner["cash_out"] += adjustment
                    biggest_winner["net_profit"] += adjustment
    ...
    # VERIFICATION: Check that all creators are in their games
    print(f"\nCreator verification:")
    creator_issues = 0
    for table in tables:
        creator_in_players = any(p["user_id"] == table["creator_id"] for p in table["players"])
        if not creator_in_players:
            print(f"ERROR: Creator {table['creator_id']} not in players for table {table['_id']}")
            creator_issues += 1

    for game in games:
        creator_in_players = any(p["user_id"] == game["creator_id"] for p in game["players"])
        if not creator_in_players:
            print(f"ERROR: Creator {game['creator_id']} not in players for game {game['_id']}")
            creator_issues += 1

    if creator_issues == 0:
        print("✓ All creators are properly included in their games!")
    else:
        print(f"✗ Found {creator_issues} creator issues!")
