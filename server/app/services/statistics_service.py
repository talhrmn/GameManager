...
class StatisticsService(BaseService[StatisticsBase, StatisticsDBOutput]):
    ...
    async def create_user_stats(self, user_id: PyObjectId) -> Optional[StatisticsDBOutput]:
        """
        Create a new users stats.

        Args:
            user_id: The user id to create

        Returns:
            Optional[StatisticsDBOutput]: The created user stats if successful, None otherwise
        ...
        """
        try:
            user_stats = StatisticsBase(user_id=str(user_id))
            return await self.repository.create(user_stats)
        except Exception as e:
    ...
    async def update_user_stats(self, user_id: str, user_inc: Stats) -> Optional[StatisticsDBOutput]:
        ...
        Returns:
            Optional[StatisticsDBOutput]: The updated stats if successful, None otherwise
